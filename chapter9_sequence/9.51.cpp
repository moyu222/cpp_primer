...
Date::Date(const std::string &s) : Date()
{
    if (s.find_first_of(alphabets) != std::string::npos)
    {
        ...
        std::string sub = s.substr(s.find_first_of("0123456789")); // 31, 1990
        day = std::stoi(sub);
        sub = sub.substr(sub.find_first_not_of("0123456789")); // ,1990
        sub = sub.substr(sub.find_first_of("0123456789")); // 1990
        year = std::stoi(sub);
    }
    else if (s.find_first_of('/') != std::string::npos)
    {
        // dd mm yyyy  31/12/1990
        std::string sub = s.substr(s.find_first_of("0123456789")); // 31/12/1990
        day = std::stoi(sub);
        sub = sub.substr(sub.find_first_not_of("0123456789")); // /12/1990
        sub = sub.substr(sub.find_first_of("0123456789")); // 12/1990
        month = std::stoi(sub);
        sub = sub.substr(sub.find_first_not_of("0123456789")); // /1990
        sub = sub.substr(sub.find_first_of("0123456789")); // 1990
        year = std::stoi(sub);
